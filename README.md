...
服务只监听 `127.0.0.1:3927`。终端会持续显示模型准备状态，并输出本地管理页地址和一条带本机访问 token 的 `claude mcp add` 命令。管理页可以筛选所有项目的记忆，查看状态、时间退化、来源、版本历史以及合并和派生关系。保持该终端运行；按 `Ctrl+C` 会安全关闭模型与数据库，已记录的数据和下载进度保留在 `~/.banana-memory`。
