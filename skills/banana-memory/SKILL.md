...
Call `record` after a durable fact becomes clear rather than waiting for every conversation to end. Include a concise, stable `projectName` based on the project or product rather than the current task or session; this is display metadata and never a workspace selector. Call `feedback` only for an observed outcome. These calls are model-mediated evidence; never describe them as direct user confirmation.
