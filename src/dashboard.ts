...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:14px/1.55 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    h1{font-family:Georgia,"Songti SC",serif;font-size:clamp(32px,4vw,52px);font-weight:500;letter-spacing:-.045em;line-height:1.08;margin:6px 0 8px}.model{display:flex;align-items:center;gap:8px;margin-top:10px;color:var(--muted)}.model i{width:8px;height:8px;border-radius:50%;background:var(--green)}.model-details{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px}.model-chip{display:inline-flex;align-items:center;gap:6px;padding:5px 9px;border:1px solid var(--line);border-radius:9px;background:rgba(255,254,249,.72);color:#55574d;font-size:11px}.model-chip b{color:var(--ink)}.model-chip em{padding-left:6px;border-left:1px solid var(--line);color:var(--green);font-style:normal}
...
(() => {
  ...
  function renderModels(model) {
    const definitions=[['generation','生成模型',model.generationLoaded?'已加载':'按需加载'],['embedding','向量模型',model.embeddingLoaded?'已加载':'未加载'],['runtime','本地运行时','']];
    const rows=definitions.filter(item=>model.models&&model.models[item[0]]).map(item=>{const value=model.models[item[0]];const title=value.repository+' · '+value.filename+' · '+value.revision;return '<span class="model-chip" title="'+esc(title)+'"><b>'+item[1]+'</b>'+esc(value.name)+(item[2]?'<em>'+item[2]+'</em>':'')+'</span>';});
    $('model-details').innerHTML=rows.join('');
  }
  async function load() {
